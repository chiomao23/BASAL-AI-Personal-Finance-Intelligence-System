...
## Files
- `BASAL Landing Page (standalone).html` — the marketing landing page (hero, features,
  how-it-works, security/CTA, footer). Embeds the dashboard as a live, clickable demo
  and links to it — keep both files in the same folder.
- `BASAL Dashboard (standalone).html` — the interactive design, fully self-contained
  (no other files needed). **Double-click to open it in any browser** to see and click the
  real prototype. Read it as the visual/behavior spec.
